...
OpenproThread::OpenproThread(const QString &src_path, int file_count, QTreeWidget *self, QObject *parent)
...
void OpenproThread::OpenProTree(const QString &src_path, int &file_count, QTreeWidget *self)
{
    //创建根节点
...
void OpenproThread::run()
{
    ...
    emit SigFinishProgress();
}

void OpenproThread::RecursiveProTree(const QString &src_path, int &file_count, QTreeWidget *self, QTreeWidgetItem *root, QTreeWidgetItem *parent, QTreeWidgetItem *preitem)
{
    QDir src_dir(src_path);
    //设置文件过滤器
...
void OpenproThread::SlotCancelProgress()
{
    this->_bstop=true;
}
