...
{
    CreateProTree(_src_path,_dist_path,_parent_item,_file_count,_self,_root,_root);
    if(_bstop){
        //停止会把目的文件下的，即root对于文件夹中的文件全部删除
...
//                qDebug() << "file src to dist  copy failed" << endl;
            ...
            preItem = item;
            if(item->GetPreItem()!=nullptr){
             qDebug() << item->GetPreItem()->Getpath()<<endl;
            }
         }
    }
